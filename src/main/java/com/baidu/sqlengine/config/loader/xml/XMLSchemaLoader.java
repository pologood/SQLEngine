...
public class XMLSchemaLoader implements SchemaLoader {
    ...
    private final static String DEFAULT_DTD = "/dtd/schema.dtd";
    ...
    private Map<String, TableConfig> loadTables(Element node) {
        ...
        for (int i = 0; i < nodeList.getLength(); i++) {
            ...
            String primaryKey =
                    tableElement.hasAttribute("primaryKey") ? tableElement.getAttribute("primaryKey").toUpperCase()
                            : null;
    ...
    private void processChildTables(Map<String, TableConfig> tables,
        ...
        for (int j = 0; j < childNodeList.getLength(); j++) {
            ...
            String primaryKey =
                    childTbElement.hasAttribute("primaryKey") ? childTbElement.getAttribute("primaryKey").toUpperCase()
                            : null;
        ...
        switch (suitValue) {
            case -1:
                ...
                throw new ConfigException("Illegal table conf : table [ " + tableConf.getName() + " ] rule function [ "
                        + tableConf.getRule().getFunctionName() + " ] partition size : " + tableConf.getRule()
                        .getRuleAlgorithm().getPartitionNum() + " > table datanode size : "
                        + tableConf.getDataNodes().size()
            ...
            case 1:
                // 有些节点是多余的,给出warn log
                LOGGER.warn(
                        "table conf : table [ {} ] rule function [ {} ] partition size : {} < table datanode size : "
                                + "{} , this cause some datanode to be redundant",
                        new String[] {
                                tableConf.getName(),
    ...
    private void loadDataNodes(Element root) {
        ...
        for (int i = 0, n = list.getLength(); i < n; i++) {
            ...
            String[] dnNames = com.baidu.sqlengine.util.SplitUtil.split(dnNamePre, ',', '$', '-');
            String[] databases = com.baidu.sqlengine.util.SplitUtil.split(databaseStr, ',', '$', '-');
            String[] hostStrings = com.baidu.sqlengine.util.SplitUtil.split(host, ',', '$', '-');
    ...
    private DBHostConfig createDBHostConf(String dataHost, Element node,
                                          String dbType, String dbDriver, int maxCon, int minCon, String filters,
                                          long logTime) {
    ...
    private void loadDataHosts(Element root) {
        ...
        for (int i = 0, n = list.getLength(); i < n; ++i) {
            ...
            boolean tempReadHostAvailable =
                    !tempReadHostAvailableStr.equals("") && Integer.parseInt(tempReadHostAvailableStr) > 0;
            /**
             * 读取 写类型
            ...
            for (int w = 0; w < writeDbConfs.length; w++) {
                ...
                if (readNodes.getLength() != 0) {
                    ...
                    for (int r = 0; r < readDbConfs.length; r++) {
                        ...
                        readDbConfs[r] =
                                createDBHostConf(name, readNode, dbType, dbDriver, maxCon, minCon, filters, logTime);
                    }
