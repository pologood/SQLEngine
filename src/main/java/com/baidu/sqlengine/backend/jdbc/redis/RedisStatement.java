...
public class RedisStatement implements Statement
{
    ...
    //int _maxRows = 0;
    private RedisResultSet _last;

    public RedisStatement(RedisConnection conn, int type, int concurrency, int holdability)
    {
        this._conn = conn;
        this._type = type;
	...
	public ResultSet executeQuery(String sql) throws SQLException {
		...
		RedisData redis= new RedisSQLParser(this._conn.getDB(), sql).query();
// fixme
//        if ((this._fetchSize > 0)
//				&& (redis.getCursor()!=null)) {
//        	//设置每次网络请求的最大记录数
//        	redis.getCursor().batchSize(this._fetchSize);
//        }
        /* 
        if (this._maxRows > 0)
        {
        ...
        */
        this._last = new RedisResultSet(redis,this._conn.getSchema());
		return this._last;
	}
	...
	public int executeUpdate(String sql) throws SQLException {
		// 执行更新语句
		return new RedisSQLParser(this._conn.getDB(), sql).executeUpdate();
	}
