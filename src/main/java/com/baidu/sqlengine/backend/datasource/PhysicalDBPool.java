...
import com.baidu.sqlengine.backend.mysql.nio.handler.GetConnectionHandler;
...
public class PhysicalDBPool {
    ...
    private boolean initSource(int index, PhysicalDatasource ds) {
        ...
        LOGGER.info(
                "init backend myqsl source ,create connections total " + initSize + " for " + ds.getName() + " index :"
                        + index);
    ...
    public PhysicalDatasource randomSelect(ArrayList<PhysicalDatasource> okSources) {
        ...
        if (okSources.isEmpty()) {
            ...
            for (int i = 0; i < length; i++) {
                ...
                if (sameWeight && i > 0
                        && weight != okSources.get(i - 1).getConfig()
     ...
     * @param includeCurWriteNode      if include current active write node. invalid when <code>includeWriteNode<code>
     *                                 is false
