...
import com.baidu.sqlengine.backend.mysql.protocol.AuthPacket;
import com.baidu.sqlengine.backend.mysql.protocol.CommandPacket;
...
import com.baidu.sqlengine.backend.mysql.protocol.QuitPacket;
...
public class MySQLConnection extends BackendAIOConnection {
    ...
    private static long initClientFlags() {
        ...
        boolean usingCompress = SqlEngineServer.getInstance().getConfig().getSystem().getUseCompression() == 1;
        if (usingCompress) {
    ...
    public MySQLConnection(NetworkChannel channel, boolean fromSlaveDB) {
        ...
        this.txIsolation = SqlEngineServer.getInstance().getConfig().getSystem().getTxIsolation();
    }
    ...
    public void release() {
        if (metaDataSyned == false) {
            // indicate connection not normalfinished
            // ,and
