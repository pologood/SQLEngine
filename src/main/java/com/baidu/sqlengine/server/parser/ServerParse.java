...
public final class ServerParse {
    ...
    private static final Pattern pattern =
            Pattern.compile("(load)+\\s+(data)+\\s+\\w*\\s*(infile)+", Pattern.CASE_INSENSITIVE);
    private static final Pattern callPattern =
            Pattern.compile("\\w*\\;\\s*\\s*(call)+\\s+\\w*\\s*", Pattern.CASE_INSENSITIVE);
        ...
        if (stmt.length() > ++offset) {
            switch (stmt.charAt(offset)) {
                ...
                case 't':
                    if (stmt.length() > ++offset) {
                        //支持一下语句
                        //  /*!sqlEngine: sql=SELECT * FROM test where id=99 */set @pin=1;
                        //                    call p_test(@pin,@pout);
                        //                    select @pout;
                        if (stmt.startsWith("/*!sqlEngine:") || stmt.startsWith("/*#sqlEngine:") || stmt
                                .startsWith("/*sqlEngine:")) {
