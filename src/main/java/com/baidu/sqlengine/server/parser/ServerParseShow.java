package com.baidu.sqlengine.server.parser;
...
public final class ServerParseShow {
    ...
    public static final int SQL_ENGINE_STATUS = 3;
    public static final int SQL_ENGINE_CLUSTER = 4;
    ...
    // SHOW SQL_ENGINE_
    static int sqlEngineCheck(String stmt, int offset) {
        if (stmt.length() > offset + "ycat_?".length()) {
            ...
            if ((c1 == 'Y' || c1 == 'y') && (c2 == 'C' || c2 == 'c')
                ...
                switch (stmt.charAt(++offset)) {
                    case 'S':
                    case 's':
                        return showSqlEngineStatus(stmt, offset);
                    case 'C':
                    case 'c':
                        return showSqlEngineCluster(stmt, offset);
                    default:
                        return OTHER;
                }
            }
    ...
    // SHOW SqlEngine_STATUS
    static int showSqlEngineStatus(String stmt, int offset) {
        if (stmt.length() > offset + "tatus".length()) {
            ...
            if ((c1 == 't' || c1 == 'T')
                    ...
                    .charAt(offset)))) {
                return SQL_ENGINE_STATUS;
            }
        }
        return OTHER;
    }

    // SHOW SqlEngine_CLUSTER
    static int showSqlEngineCluster(String stmt, int offset) {
    ...
    private static Pattern fullpattern =
            Pattern.compile("^\\s*(SHOW)\\s+(FULL)+\\s+(TABLES)\\s+\\s*([\\!\\'\\=a-zA-Z_0-9\\s]*)",
