...
import com.baidu.sqlengine.memory.unsafe.array.ByteArrayMethods;
import com.baidu.sqlengine.memory.unsafe.memory.MemoryAllocator;
...
public abstract class MemoryManager {
    ...
    @GuardedBy("this")
    protected ResultSetMemoryPool onHeapExecutionMemoryPool = new ResultSetMemoryPool(this, MemoryMode.ON_HEAP);

    @GuardedBy("this")
    protected ResultSetMemoryPool offHeapExecutionMemoryPool = new ResultSetMemoryPool(this, MemoryMode.OFF_HEAP);

    protected long maxOffHeapMemory = 0L;
    protected long offHeapExecutionMemory = 0L;
    private int numCores = 0;

    public MemoryManager(SqlEnginePropertyConf conf, int numCores, long onHeapExecutionMemory) {
        this.conf = conf;
        this.numCores = numCores;
        maxOffHeapMemory = conf.getSizeAsBytes("sqlEngine.memory.offHeap.size", "128m");
    ...
    public final MemoryMode tungstenMemoryMode() {
        if (conf.getBoolean("sqlEngine.memory.offHeap.enabled", false)) {
            assert (conf.getSizeAsBytes("sqlEngine.memory.offHeap.size", 0) > 0);
     ...
     * <p/>
     * If user didn't explicitly set "sqlEngine.buffer.pageSize", we figure out the default value
    ...
    public long pageSizeBytes() {
        ...
        defaultSize = conf.getSizeAsBytes("sqlEngine.buffer.pageSize", defaultSize);
