...
public class DataNodeMemoryManager {
    ...
    public long acquireExecutionMemory(long required, MemoryMode mode, MemoryConsumer consumer)
        ...
        synchronized(this) {
                    ...
                    if (c != consumer && c.getUsed() > 0) {
                        try {
                            ...
                            if (released > 0 && mode == tungstenMemoryMode) {
                                logger.info("Thread " + connectionAttemptId + " released " + JavaUtils
                                        .bytesToString(released) +
                                        " from " + c + " for" + consumer);
            ...
            // logger.info("Thread" + connectionAttemptId + " acquire "+  JavaUtils.bytesToString(got) +" for "+
            // consumer+"");
            return got;
        }
    }
    ...
    public void showMemoryUsage() {
        ...
        synchronized(this) {
            ...
            long memoryNotAccountedFor =
                    memoryManager.getExecutionMemoryUsageForConnection(connectionAttemptId)
