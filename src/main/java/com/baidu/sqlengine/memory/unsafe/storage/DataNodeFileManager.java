 ...
 * <p/>
 * Block files are hashed among the directories listed in sqlEngine.local.dir
 */
public class DataNodeFileManager {
    ...
    public DataNodeFileManager(SqlEnginePropertyConf conf, boolean deleteFilesOnStop) throws IOException {

        this.conf = conf;
        this.deleteFilesOnStop = deleteFilesOnStop;

        subDirsPerLocalDir = conf.getInt("sqlEngine.diskStore.subDirectories", 64);
     ...
     */
    private List<File> createLocalDirs(SqlEnginePropertyConf conf) {

        String rootDirs = conf.getString("sqlEngine.local.dirs", "datanode");
