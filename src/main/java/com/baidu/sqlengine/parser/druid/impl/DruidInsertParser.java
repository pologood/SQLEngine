...
public class DruidInsertParser extends DefaultDruidParser {
    ...
    private RouteResultSet parserChildTable(SchemaConfig schema, RouteResultSet rrs,
                                            String tableName, MySqlInsertStatement insertStmt)
            throws SQLNonTransientException {
        TableConfig tc = schema.getTables().get(tableName);
        ...
        RouteResultSet theRrs =
                RouterUtil.routeByERParentKey(null, schema, ServerParse.INSERT, sql, rrs, tc, joinKeyVal);
        if (theRrs != null) {
            rrs.setFinishedRoute(true);
            return theRrs;
        }
    ...
    private void parserSingleInsert(SchemaConfig schema, RouteResultSet rrs, String partitionColumn,
        ...
        for (int i = 0; i < insertStmt.getColumns().size(); i++) {
            if (partitionColumn
                    .equalsIgnoreCase(StringUtil.removeBackquote(insertStmt.getColumns().get(i).toString()))) {//找到分片字段
                isFound = true;
