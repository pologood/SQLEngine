...
import com.baidu.sqlengine.memory.unsafe.KVIterator;
import com.baidu.sqlengine.memory.unsafe.map.UnsafeFixedWidthAggregationMap;
...
import com.baidu.sqlengine.memory.unsafe.utils.BytesTools;
...
import com.baidu.sqlengine.util.ByteUtil;
...
public class UnsafeRowGrouper {
    ...
    public UnsafeRowGrouper(Map<String, ColMeta> columToIndx, String[] columns, MergeCol[] mergCols,
                            HavingCols havingCols) {
        super();
        assert columns != null;
        ...
        for (int i = 0; i < this.groupKeyfieldCount; i++) {
            ...
            if (!row.isNullAt(curColMeta.colIndex)) {
                switch (curColMeta.colType) {
                    ...
                    case ColMeta.COL_TYPE_DOUBLE:
                    case ColMeta.COL_TYPE_NEWDECIMAL:
                        key.setDouble(curColMeta.colIndex, BytesTools.getDouble(row.getBinary(curColMeta.colIndex)));
                        break;
                    case ColMeta.COL_TYPE_LONGLONG:
            ...
            if (!row.isNullAt(curColMeta.colIndex)) {
                switch (curColMeta.colType) {
                    ...
                    case ColMeta.COL_TYPE_LONGLONG:
                        ...
                        logger.error("getValue COL_TYPE_LONGLONG ===> " + "size  " + row
                                .getBinary(curColMeta.colIndex).length + "  " +
                    ...
                    case ColMeta.COL_TYPE_FLOAT:
                        ...
                        logger.error(
                                "getValue COL_TYPE_FLOAT " + BytesTools.getFloat(row.getBinary(curColMeta.colIndex)));
